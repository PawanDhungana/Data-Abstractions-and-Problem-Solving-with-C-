...
class Concordance
{
    ...
    // MODIFICATION MEMBER FUNCTION
      void insert ( const Word& word, int& count );
...
void Concordance::insert(const Word& word, int& count)
{
	Node *temp;
	...
	else
	{
		...
		temp = temp->next;
		temp->next = get_node(word,count,temp->next);
	}
} 
...
void read_word(ifstream& in_f, char (&array)[MAX+1])
{
	char ch;
	int i=0;
	in_f.get(ch);
